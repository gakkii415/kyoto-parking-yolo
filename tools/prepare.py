"""Prepare real GSI aerial images and official YOLO11n-OBB weights (AGPL-3.0)."""
...
def mosaic(site,layer='seamlessphoto'):
 ...
 def tile(xy):
  x,y=xy; url=f'https://cyberjapandata.gsi.go.jp/xyz/{layer}/{z}/{x}/{y}.{ext}'
  key=CACHE/f'{layer}-{z}-{x}-{y}.{ext}'
  if not key.exists(): key.write_bytes(get(url))
  im=Image.open(key).convert('RGBA')
  if im.size!=(256,256) or im.getchannel('A').getextrema()[0]<255: raise ValueError('Incomplete imagery coverage')
  return x,y,im,url
 canvas=Image.new('RGB',(size,size)); urls=[]
 with ThreadPoolExecutor(max_workers=4) as pool:
  for x,y,im,url in pool.map(tile,tiles):
   canvas.paste(im.convert('RGB'),(x*256-left,y*256-top)); urls.append(url)
 name=f"{site['id']}-{layer}"; canvas.save(ASSETS/f'{name}.jpg',quality=96)
 return canvas,{'id':name,'siteId':site['id'],'name':site['name'],'district':site['district'],
  'image':f'assets/{name}.jpg','width':size,'height':size,'latitude':site['lat'],'longitude':site['lon'],
  'zoom':z,'globalPixelOrigin':[left,top],'layer':layer,
  'source':'国土地理院・地理院タイル（切り出し・結合）','sourceUrl':'https://maps.gsi.go.jp/development/ichiran.html',
  'sourceTiles':urls,'acquiredAt':None,'retrievedAt':NOW,'captureLabel':'撮影日不明・配信写真',
  'sha256':hashlib.sha256((ASSETS/f'{name}.jpg').read_bytes()).hexdigest(),'roi':[[0,0],[1,0],[1,1],[0,1]]}
...
classes={int(k):v.replace(' ','-') for k,v in model.names.items()}
...
metadata={'name':'YOLO11n-OBB','task':'obb','inputSize':640,'classes':classes,'vehicleIds':vehicle_ids,
 'outputShape':list(shape),'license':'AGPL-3.0','source':'https://github.com/ultralytics/assets/releases/tag/v8.3.0',
 'weightsSha256':hashlib.sha256(weights.read_bytes()).hexdigest(),'onnxSha256':hashlib.sha256(model_path.read_bytes()).hexdigest(),
...
for site in SITES:
 ...
 for xywhr,conf,cls in zip(pred.obb.xywhr.tolist(),pred.obb.conf.tolist(),pred.obb.cls.tolist()):
  detections.append({'cx':xywhr[0],'cy':xywhr[1],'w':xywhr[2],'h':xywhr[3],'angle':xywhr[4],'score':conf,'classId':int(cls)})
 record['preview']={'engine':'YOLO11n-OBB / Python','generatedAt':NOW,'detections':detections,'confidence':.15}
 Image.fromarray(pred.plot()[:,:,::-1]).save(ASSETS/f"{site['id']}-preview.jpg")
 examples.append(record); print('SAMPLE',site['id'],'ACTUAL_DETECTIONS',len(detections),flush=True)
