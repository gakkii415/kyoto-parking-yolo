...
https://gakkii415.github.io/kyoto-parking-yolo/?v=5
...
デザインはユーザー指定Superhumanへ更新。DESIGN.mdのProject Directionを正本として、羊皮紙色の背景、ワインの主操作、薄紫タブ、軽い透過パネルを適用。全体再評価で採用したモード切替時の再生操作抑止、人専用骨格の対象表示、先読みなし開始を反映済み。新たに採用する改善0件。実画面検証は未実施。

### v5 tracking continuity
未検出トラックを映像時間3秒保持。最後の観測軌跡を薄く表示し、未検出対象は数えない。同種・近距離・移動方向で再接続し、同じIDを二重割当しない。跡は最大240観測点。一時停止では履歴を保持し、シーク・動画変更・モード変更ではリセット。見失い、復帰、期限、遠方・異種除外、停止、二重割当を回帰試験。人物の外観による同一性判定ではないため、交差・長い遮蔽ではIDが変わり得る。
