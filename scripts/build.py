...
for key,name,t in [('store','store-aisle-detection',35),('road','car-detection',5),('street','person-bicycle-car-detection',5)]:
 ...
 video.set(cv2.CAP_PROP_POS_MSEC,min(t,duration/2)*1000);ok,frame=video.read();assert ok;video.release()
 ...
 checks.append({'scene':key,'geometry':{'scale':scale,'padX':px,'padY':py,'width':nw,'height':nh},'duration':duration})
