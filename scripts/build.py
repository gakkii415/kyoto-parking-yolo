...
# GSI z18 aerial photograph, Kyoto Iwakura parking area.
z=18; x=229948; y=103778
url=f'https://cyberjapandata.gsi.go.jp/xyz/seamlessphoto/{z}/{x}/{y}.jpg'
raw=urllib.request.urlopen(url,timeout=60).read()
img=Image.open(BytesIO(raw)).convert('RGB').crop((0,128,128,256));img.save(out/'sample.png')
checkpoint_url='https://huggingface.co/dronefreak/visdrone-yolov11s/resolve/853d168/best.pt'
urllib.request.urlretrieve(checkpoint_url,'visdrone.pt')
model=YOLO('visdrone.pt')
assert [model.names[i] for i in [3,4,5,8]]==['car','van','truck','bus'],model.names
...
a=np.array(img.resize((640,640),Image.Resampling.BILINEAR),dtype=np.float32).transpose(2,0,1)[None]/255
pred=session.run(None,{session.get_inputs()[0].name:a})[0]
assert pred.shape==(1,15,8400),pred.shape
print('CLASS_MAX',[(model.names[i],float(pred[0,4+i].max())) for i in range(11)])
r=model.predict(img,imgsz=640,conf=.05,verbose=False)[0]
print('REFERENCE',r.boxes.data.tolist())
# Keep raw outputs so the exact same browser decoder is tested during CI.
(out/'raw.bin').write_bytes(pred.astype('<f4').tobytes())
meta={'name':'京都・岩倉の駐車場','lat':35.062696,'lon':135.785897,'source':url,'captureDate':None,'retrievedAt':datetime.now(timezone.utc).isoformat(),'model':'YOLO11s / VisDrone','width':128,'height':128,'crop':[0,128,128,256],'roi':[[46,88],[91,90],[82,127],[39,122]],'input':640}
